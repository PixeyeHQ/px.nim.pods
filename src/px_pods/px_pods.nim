...
template fatal*(errorKind: PodErrorKind, args: varargs[string]) =
  when defined debug:
    ...
    when compiles(pods_io.debugProc(message)):
      pods_io.debugProc(message)
    else:
      echo message
...
template fatal*(tag, message: string) =
  when defined debug:
    block:
...
proc toPodString*(pod: var Pod, style: PodStyle = PodStyle.Sparse): string =
  const bufferLen = 64
  var p: PodWriter
...
proc toPodString*(pod: Pod, style: PodStyle = PodStyle.Sparse): string =
  var refPod      = pod
  var refSettings = style
  toPodString(refPod, refSettings)


proc toPodFile*(path: string, pod: Pod, style: PodStyle = PodStyle.Sparse) =
  var podstring = toPodString(pod, style)
